...
\modMF{
\subsection{Detailed example on the bacteriophage lambda immunity control model}
}
...
and 3 cooperative sorts: %($\PHs \setminus \Gamma = \{ \cIcrocII, \cIcroN, \cIcro \}$).
...
\begin{tikzpicture}
  ...
  \TSetTick{cIcro}{1}{\ttable{FT}{<}{\leq}}
  \TSetTick{cIcro}{2}{\ttable{TF}{\leq}{<}}
  \TSetTick{cIcro}{3}{\ttable{TT}{\leq}{\leq}}
...
\begin{align*}
  ...
  \irB_\N^{\{ \cI, \cro \}}(\PHetat{\cI_2, \cro_3, \N_0}) &= \{ \N_0 \} && &&
    \text{(from \pref{eq:B-bounce})}
\end{align*}
Therefore, \pref{eq:inference-edges-ba} gives: $\cI \xrightarrow{1} \N \in \hat{E}_-$.
...
However, we note that all processes of $\cI$ can hit $\cro_1$ and $\cro_2$;
therefore $\Phi(\cro_1) = \Phi(\cro_2) = \emptyset$, and:
%, there exist no sub-state $\sigma \in \Phi(\cro_1)$
\begin{align*}
  \irB_{\cro}^{\{ \cro \}}(\PHetat{\cro_3}) &= \{ \cro_2 \} && &&
    \text{(from \pref{eq:B-bounce})} \\
  \irB_{\cro}^{\{ \cro \}}(\PHetat{\cro_2}) &= \emptyset &&
    \text{because } \Phi(\cro_2) = \emptyset &&
    \text{(from \pref{eq:B-no-bounce})} \\
  \irB_{\cro}^{\{ \cro \}}(\PHetat{\cro_1}) &= \emptyset &&
    \text{because } \Phi(\cro_1) = \emptyset &&
    \text{(from \pref{eq:B-no-bounce})} \\
  \irB_{\cro}^{\{ \cro \}}(\PHetat{\cro_0}) &= \{ \cro_0 \} &&
    \text{because } \PHetat{\cI_2, \cro_0} \in \Phi(\cro_0) &&
    \text{(from \pref{eq:B-no-bounce})}
\end{align*}

We thus cannot infer any local influence by observing the group of regulators $\{ \cro \}$.
...
Furthermore:
\begin{align*}
%   \irB_{\cro}^{\{ \cI, \cro \}}(\PHetat{\cI_0, \cro_0}) &= \{ \cro_1 \} &&
%   \text{(from \pref{eq:B-bounce})} \\
%   \irB_{\cro}^{\{ \cI, \cro \}}(\PHetat{\cI_0, \cro_1}) &= \{ \cro_2 \} &&
%   \text{(from \pref{eq:B-bounce})} \\
  ...
  \irB_{\cro}^{\{ \cI, \cro \}}(\PHetat{\cI_0, \cro_3}) &= \{ \cro_2 \} &&
    \text{(from \pref{eq:B-bounce})}
\end{align*}
Therefore, \pref{eq:inference-edges-aa} gives: $\cro \xrightarrow{3} \cro \in \hat{E}_-$.
% The configurations using the other processes of $\cro$ ($\cro_0$, $\cro_1$ and $\cro_2$)
% do not add more information on the influences towards $\N$.
