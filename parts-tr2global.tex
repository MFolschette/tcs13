...
\todo{introduce \pref{ssec:split-sorts}}
...
\modLP{Assuming a PH and a partition $\Gamma$ and $\CSorts$ of its sort that satisfy
\pref{pro:wf-ph}, }
\pref{def:ph-pred} characterizes the set of predecessors of a sort $a$, that is, the sorts influencing $a$
...
\todo{update text and improve definition}
...
$\sigma\in L(g)$
\todo{$L(g\cup \{a\})$ ?},
$\ctx_a^g(\sigma)$ is the configuration $\sigma$ with the corresponding
