...
For the PH in \pref{fig:runningPH-2}, this latter is evaluated to $\segm{1}{1}$.
\end{example*}
...
\begin{itemize}
  ...
  \item the \aspil{param\_inf(\ti{a}, \ti{p}, \ti{q})} atom means: $K^\ti{p}_{\ti{a},A,B} \leqsegm K^\ti{q}_{\ti{a},A',B'}$,
\end{itemize}
...
which removes all parametrization results where parameters $K^\aspil{P}_{\aspil{X},A,B}$ and $K^\aspil{Q}_{\aspil{X},A',B'}$ exist such that $A \subset A'$ and $K^\aspil{Q}_{\aspil{X},A',B'} \ltsegm K^\aspil{P}_{\aspil{X},A,B}$, thus violating the monotonicity assumption.
