...
evolution in a given configuration $\sigma$ of any group of influence on $a$.
...
b\xrightarrow t a \in \hat{E}_s & \Longleftrightarrow
  ...
  \exists \sigma\in\textstyle\prod_{c\in g \cup \{ a \}} L_c, \exists b_i,b_{i+1}\in \PHl_b,\\
& \qquad\qquad
...
\exists g \in X(a), \sigma \in \textstyle\prod_{b\in g \cup \{ a \}} L_b,
...
\todo{Add a new subsection about the alternative inference (cf.~A.~Richard)}
...
\rewriteil{$B_a(\sigma)$ = Ensemble des processus vers lesquels on bondit}

\begin{align}
\begin{split}\label{eq:edges-inference-richard-pos}
  \forall b\in\PHpredecgene{a}, \qquad\qquad\\
  b\xrightarrow{t+1} a \in \hat{E}_+ \Longleftrightarrow\ &
    \exists a_j \in B_a(\sigma\{b_{t}\}), \exists a_k \in B_a(\sigma\{b_{t+1}\}),\\
...
\todo{\underline{$^*$Garder les conditions larges ?}}
