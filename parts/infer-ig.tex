...
positives and/or negatives, between the components of the system.
As discussed in \pref{sec:intro}, the IG allows to efficiently characterise global dynamical
...
However, it is very common that the prior IG actually refers interaction that reveal to be 
...
%To infer influences on a component $a$, we compare several configurations of groups of components of $a$.
%Indeed, to study the influence of any component $b$ on $a$, it is sufficient to study the (unique) group of influence
%$g \in X(a)$ which $b$ belongs to
%(that is, the unique set of components regulating $a$ through common cooperative sorts containing $b$).
%This reasoning relies on the observation of the direction of evolution of $a$ for different active processes of $b$.
%and we can infer a positive influence if raising the level of $b$ changes the evolution of $a$ 
...
\end{split}\\
%
%%% Tentative de réduire le nombre d'auto-arcs superflus
%
...
%\end{split}
%
%%% Ancienne version de l'inférence
%
%\begin{split}\label{eq:edges-inference-pos}
...
%    &\qquad \exists a_j \in \irB_a(\sigma), \exists a_k \in \irB_a(\sigma\{b_{t+1}\}), a_j < a_k
%%    &\qquad \warnul{a_j \leq \PHget{\sigma}{a} < a_k \quad^*}\\
%%    &\qquad \warnul{a_j \leq \PHget{\sigma}{a} < a_k \vee a_j < \PHget{\sigma}{a} \leq a_k \quad^{**}}\\
%%    &\qquad \warnul{a_j < a_k \quad^{***}}
%\end{split}\\
%\begin{split}\label{eq:edges-inference-neg}
%  \forall b\in\PHpredecgene{a}, \qquad\qquad\quad\\
%  b\xrightarrow{t+1} a \in \hat{E}_- \Longleftrightarrow\ &
%    \exists \sigma \in \textstyle\prod_{c\in \reg{a} \cup \{ a \}} \PHl_c, \{ b_t, b_{t+1} \} \subset \PHl_b \wedge b_t \in \sigma,\\
%    &\qquad \exists a_j \in \irB_a(\sigma), \exists a_k \in \irB_a(\sigma\{b_{t+1}\}), a_j > a_k
%%    &\qquad \warnul{a_j > \PHget{\sigma}{a} \geq a_k \quad^*}\\
%%    &\qquad \warnul{a_j > \PHget{\sigma}{a} \geq a_k \vee a_j \geq \PHget{\sigma}{a} > a_k \quad^{**}}\\
%    %&\qquad \warnul{a_j > a_k \quad^{***}}
%\end{split}
\end{align}
...
\end{proposition}
%
%\todo{\underline{$^*$Strict inference}}\\
%\todo{\underline{$^{**}$Large inference}}\\
%\todo{\underline{$^{***}$Non-unitary-style inference}}

\todo{remark for reducing the number of self-regulation}
