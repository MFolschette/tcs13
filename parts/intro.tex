...
state graph explosion, \updated{an other major} drawback of this framework is the difficulty to
...
State Machines, \updated{where the arity and the effect of synchronisations between components is
restricted.}
...
\updated{which was impossible with other state-of-the-art approaches \cite{PMR12-MSCS,PAK13-CAV,FPMR13-CS2Bio}}.
...
\updated{%
The aim of this paper is to formally estiblish links between several layers suited for specifying the
...
the Process Hitting (PH), which permits to specify the qualitatif dynamics of the system with
...
The most known are the Thomas' conjectures (that have been proved since \todo{refs}),
...
\todo{check if the inferred IG contains the IG of compatible Thomas}
...
The contributions presented in this paper significantly extends and improves the preliminary results
...
\todo{asp improvement? more detailed application?}
