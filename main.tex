...
\newcommand{\rewrite}[1]{\textcolor{blue}{\textbf{[REWRITE: #1]}}}
...
\title{
\todo{CMSB title:Concretizing the Process Hitting into Biological Regulatory Networks}
\\\rewrite{Find a new title:}
\begin{itemize}
  \item Abstracting BRNs from PH models
  \item Abstracting the BRNs of a PH model
  \item Extracting BRNs from PH models
  \item ???
\end{itemize}}
...
between components.
To handle wider class of models, we introduce unsigned edges to model regulations that are
more complex than simple activations or inhibitions.
