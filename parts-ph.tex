...
\begin{example}
\todo{\pref{fig:runningPH-1} lacks readability (arrows hitting $a$ are too small).}
...
\todo{The reviewer did not understand the following paragraph about the time shift.
He wonders if it is really important to the understanding of the rest of the paper. Three possibilities here:
\begin{enumerate}
  \item Add more (informal? formal?) explanations,
  \item Remove (or substantially reduce) this paragraph,
  \item Use priorities and prioritized cooperative sorts to remove the temporal shift.
\end{enumerate}
The last possibility would be formally the most accurate but also the most time-consuming one.}
