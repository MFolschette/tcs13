...
%A sequence of actions $h^1,\dots,h^n$ is \emph{bounce-wise} if and only if
%$\forall m\in\segm{1}{n-1}, \PHbounce(h^m)=\PHtarget(h^{m+1})$.

\todo{define $L(g) = \prod_{b\in g} L_b$}
\todo{check state notations}

\medskip

\modLP{PH can be seens as a particular sub-class of safe Petri nets \cite{BC92},
i.e. Petri nets with at most one token per place, having
groups of multually exclusive places (processes) acting as the sorts, and where each
...
processes are present, i.e., modeling conjunctions -- or cooperations -- between the presence of
several processes.
As described in \cite{PMR10-TCSB}, such cooperations can be encoded in PH with
the use of some intermediate sorts, so-called \emph{cooperative sorts}, that
