...
In addition, having both the interaction graph and the parametrization in the model allows to decouple the IG from the dynamics, which is interesting for modeling biological systems as the IG is often a prior knowledge with possible inconsistencies with the dynamics we want to encode with. A further step may then infer back the IG of the actual dynamics (here, an abstraction of the encoded dynamics) and compare it with the prior IG. Typically, if the inferred IG is included in the prior one, we have consistency.

Finally, several alternative definitions exist, with various degree of equivalency. As a consequence, we chose here to stick to the original formalism and picked this definition as it the original and standard one in the Thomas' community.

Nevertheless, to remove ambiguity and comply with the reviewer's suggestions, we redesigned section 2.2, emphasizing such a choice, especially with this excerpt:
...
This notation was intended to match the corresponding one for the Process Hitting models, but it was indeed a poor choice. We took this comment into account and changed this notation to $E^{-1}(b)$.
...
\answer
We took the second comment into account and removed the discussion about maximum levels which was indeed unnecessary.

\DISCUSS{LP} Yes, we should make clear that we are looking for monotonic
influences - but not necessarily at the global scope, which mean that we can
find two contradictory monotonic influences, leading to those ``unsigned'' edges
in the IG.
And yes, we should just mention that PH models can have an arbitrary number of
levels.

\DISCUSS{MF} In fact I don't really understand this comment --- I fist thought it was related to questions 1 and 9 but in fact it is not. There is no inference on page 11; was the reviewer talking about the dynamics? In the end, has the discussion about monotonic influences been added to the paper?
...
\answer
We took this comment into account and made major changes to this section, by splitting it in two and adding explanations and examples for most definitions.
% \DISCUSS{LP}redo this section
...
\begin{itemize}
  \item We have actualized the running examples, especially in order to fix this inconsistency between the explanations and the figure.
  \item A subsection has been added (subsection 7.1) in order to detail an example of our method on the bacteriophage lambda immunity control, as suggested.
\end{itemize}

% \DISCUSS{LP} use Lambda phage as a running example (or dedicate a subsection to
% apply the whole pipeline to it).
