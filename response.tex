...
We thank the reviewer for th comments and  suggestions.

\TODO{summary of response + changes}
...
\DISCUSS{LP}
We have to clarify this point, while insisting that we want to stick with this
standard definition (due to Thomas). Arguments:
\begin{enumerate}
\item this design allows to decouple the IG from the dynamics, which is
interesting for modelling biological systems as the IG is often a prior
knowledge with possible inconsistencies with the dynamics we want to encode
with.
A further step may then infer back the IG of the actual dynamics (here, an
abstraction of the encoded dynamics) and compare it with the prior IG. Typically, if the inferred IG is included in the prior
one, we have consistency.
\item Additional constraints (as discussed in sect XX) on parameters can also
ensure consistency between the prior IG and the encoded dynamics.
For instance, monotonicity (which imposes an ordering between the value of
parameters), but also lighter constraints (like imposing minimum and maximum
values).
Hence, the K definition is very generic, ranging from totally decoupled
specification to very constrained valuation w.r.t to the prior IG.
\item Several alternative definition exist, with various degree of equivalency.
We picked this definition as it the original and standard one in the Thomas
community (give reference to Thomas' articles).
\end{enumerate}
