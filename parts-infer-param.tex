...
When applied to the refined PH in \pref{fig:runningPH-2},
and with the IG of \pref{fig:runningBRN}(left)
...
with the IG produced by the IG inference described in \pref{ssec:infer-ig-examples}
(that is, the same as \pref{fig:runningBRN}(left)
...
because \modMF{$K_{a,\{b\}}$, $K_{a,\{c\}}$ and $K_{b,\emptyset}$} could not be inferred.
It is however possible to enumerate all \modMF{partial} and admissible parametrizations
compatible with both the inferred parameters, and the properties of this subsection.
This enumeration gives 9 different \modMF{partial} parametrizations
...
\begin{align*}
  ...
  K_{a,\{c\}} &\in \{ 0, 1, 2 \}
\end{align*}
$K_{b,\emptyset}$ is excluded
from all return parametrizations because no value is compatible with the previous properties.
}%
...
% \end{align*}
\todo{Find an example to apply the assumptions (properties 6--8)?}

Finally, we note that \modMF{$1$} belongs to the possible values for both parameters.
Therefore this enumeration allows, from the model in \pref{fig:runningPH-1},
\modMF{%
to find the behavior of component $a$ in the model refined with a cooperative sort described in
