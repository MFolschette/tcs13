...
which force each enumerated parameter to have exactly one value.
...
$K_{a,\{b\}}$, $K_{a,\{c\}}$ and $K_{b,\emptyset}$
could not be inferred by \pref{pps:param_K}.
The previous cardinality rule allows to produce 18 parametrizations,
...
Note that $0$ belongs to the set of candidate values for parameter $K_{b,\emptyset}$
because the extreme values assumption (\pref{pro:param_enum_extreme})
...
\todo{Check this example w.r.t.~\pref{pro:K-valid}.}
\end{example}
...
% \modMF{%
For example,
% , due to the fact that the parameters $K_{a,\{b\}} = \segm{1}{1}$ and $K_{a,\{c\}} = \segm{1}{1}$ could be inferred,
the extreme values assumption (\pref{pro:param_enum_extreme}) removes all parametrizations with
$K_{b,\emptyset} = 0$,
thus reducing the set of candidate values to 9 possible values:
\begin{align*}
  (K_{a,\{b\}}; K_{a,\{c\}}; K_{b,\emptyset}) \in
    \{ 0, 1, 2 \} \times \{ 0, 1, 2 \} \times \{ 1 \}
\end{align*}
Furthermore, parameter $K_{b,\emptyset} = 1$ doest not respect \pref{pro:K-valid},
because no action in the model of \pref{fig:runningPH-1} allows to bounce from $b_0$ to $b_1$.
Therefore, this parameter is removed from all returned parametrizations.
In the end, 9 partial parametrizations are therefore produced.
}%
